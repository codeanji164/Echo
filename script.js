...
/* ==========================================================================
   11. ANSWER SYSTEM
   --------------------------------------------------------------------------
   Wires up the "WHO KILLED ANJI?" answer section that already exists in
   index.html (#answer-section, #case-submission-form,
   #primary-suspect-input, #close-case-button, #case-result). This section
   is fully self-contained:
     ...
     - No existing markup, layout, or styling is changed from here.
   ========================================================================== */

const ACCEPTED_ANSWERS = ["smoochkin", "detective smoochkin"];

const caseSubmissionForm = document.getElementById("case-submission-form");
const primarySuspectInput = document.getElementById("primary-suspect-input");
...
/**
 * Handles submission of the answer form: checks the typed answer against
 * the accepted list, shows the appropriate result message, and locks the
 * input/button after an incorrect guess.
 */
function handleCaseSubmission(event) {
  event.preventDefault();

  const normalised = normaliseAnswer(primarySuspectInput.value);
  const isCorrect = ACCEPTED_ANSWERS.includes(normalised);

  if (isCorrect) {
    caseResult.textContent = "CASE SOLVED.";
  } else {
    caseResult.textContent =
      "Incorrect conclusion. The investigation has failed.";
    primarySuspectInput.disabled = true;
    closeCaseButton.disabled = true;
  }
}

caseSubmissionForm.addEventListener("submit", handleCaseSubmission);
