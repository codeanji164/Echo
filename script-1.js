...
/* ==========================================================================
   11. ANSWER SYSTEM
   --------------------------------------------------------------------------
   Adds a simple, always-visible "Who killed Anji?" answer section below
   the main story area. This section is fully self-contained:
     - It does not read from or modify storyData / storyState.
     - It does not read from or modify evidenceData / evidenceState.
     - It does not add timers or memory progression.

   Since no HTML markup was provided alongside this script, the required
   elements (heading, text input, submit button, result message) are
   created here at runtime and inserted directly after the existing
   #continue-button, i.e. below the main story area, without altering any
   existing markup, layout, or styling rules. The "app-button" class is
   reused on the submit button so it visually matches the rest of the app.
   ========================================================================== */

const ACCEPTED_ANSWERS = ["smoochkin", "detective smoochkin"];

/**
 * Normalises a raw answer string for comparison: lowercases it, trims
 * leading/trailing whitespace, and collapses any run of internal
 * whitespace down to a single space.
 */
function normaliseAnswer(rawAnswer) {
  return rawAnswer.toLowerCase().trim().replace(/\s+/g, " ");
}

/**
 * Builds the answer section DOM structure and inserts it immediately
 * after the existing #continue-button, keeping it below the main story
 * area without disturbing any existing elements.
 */
function buildAnswerSection() {
  ...
  heading.id = "answer-heading";
  heading.textContent = "WHO KILLED ANJI?";

  const answerInput = document.createElement("input");
  answerInput.type = "text";
  answerInput.id = "answer-input";
  answerInput.setAttribute("placeholder", "Type your answer...");
  ...
  resultMessage.id = "answer-result";

  function handleSubmitAnswer() {
    const normalised = normaliseAnswer(answerInput.value);
    const isCorrect = ACCEPTED_ANSWERS.includes(normalised);
  ...
  answerSection.appendChild(resultMessage);

  // Insert below the main story area (right after the Continue button)
  // without modifying any existing element or its attributes.
